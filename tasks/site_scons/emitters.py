...
def python_emitter(target, source, env):
    ...
    """
    # run pylint if PYLINT is set to True
    if env.get("PYLINT", False):
        lint_files = []
        for source_file in source:
            if ".py" == os.path.splitext(str(source_file))[-1]:
                lint_file = str(source[0]).replace(".py", ".lint")
                os.system(f"pylint {source_file} > {lint_file}")
                lint_files.append(lint_file)
        target = target + lint_files
